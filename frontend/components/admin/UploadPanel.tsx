...
export function UploadPanel({ endpoint, label, accept, pollAsset = true, initialAssetId, onReady, onUploaded }: UploadPanelProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [uploading, setUploading] = useState(false);
  const [progress, setProgress] = useState(0);
  const [assetId, setAssetId] = useState<string | null>(initialAssetId ?? null);
  const [asset, setAsset] = useState<AdminAssetStatus | null>(initialAssetId ? { status: "pending", qualities: [], error: null } : null);
  ...
  return (
    <section className="min-w-0 rounded-xl border border-zinc-700 bg-zinc-900/70 p-4 sm:p-5">
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row">
        <input ref={inputRef} type="file" accept={accept} className="max-w-full min-w-0 flex-1 rounded border border-zinc-600 bg-black p-2 text-base file:mr-3 file:max-w-full file:rounded file:border-0 file:bg-white file:px-3 file:py-2 file:font-bold file:text-black" />
