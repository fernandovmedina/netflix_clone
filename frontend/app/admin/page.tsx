  ...
  return <div>
    ...
    {!loading && !error && titles.length > 0 && <div className="mt-9 max-w-full overflow-x-auto rounded-xl border border-zinc-800">
      ...
      <div className="divide-y divide-zinc-800">{titles.map((item) => {
        ...
        const editId = kind === "movies" ? item.movie_id ?? item.title_id ?? item.id : item.title_id ?? item.id;
        const status = item.asset_status ?? (item.asset_id ? "ready" : "draft");
        return <article key={item.title_id ?? item.id} className="grid min-w-0 grid-cols-[4rem_minmax(0,1fr)_auto] items-center gap-3 bg-zinc-950 px-4 py-4 hover:bg-zinc-900 sm:gap-4 md:grid-cols-[5rem_minmax(0,1fr)_9rem_8rem_5rem] md:px-5">
          ...
          <Link href={`/admin/${kind}/${editId}`} className="hidden rounded border border-zinc-600 px-3 py-2 text-center text-sm font-bold hover:border-white md:block">Edit</Link>
          <Link href={`/admin/${kind}/${editId}`} className="col-span-3 rounded border border-zinc-700 py-2 text-center text-sm md:hidden">Edit title</Link>
        </article>;
      })}</div>
    </div>}
  </div>;
