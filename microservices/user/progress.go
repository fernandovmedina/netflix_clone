...
func (app *application) getProgress(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if kind == "movie" {
		err = app.pool.QueryRow(r.Context(), `select wp.current_time_seconds,wp.updated_at from watch_progress wp
			join movies m on m.id_movie=wp.id_movie and m.deleted_at is null
			join titles t on t.id_title=m.id_title and t.deleted_at is null and t.published=true
			where wp.user_id=$1 and wp.id_movie=$2 and exists(select 1 from video_assets va where va.id_movie=m.id_movie and va.status='ready')`, userID(r), id).Scan(&out.CurrentTime, &out.UpdatedAt)
	} else {
		err = app.pool.QueryRow(r.Context(), `select wp.current_time_seconds,wp.updated_at from watch_progress wp
			join episodes e on e.id_episode=wp.id_episode and e.deleted_at is null
			...
			where wp.user_id=$1 and wp.id_episode=$2 and exists(select 1 from video_assets va where va.id_episode=e.id_episode and va.status='ready')`, userID(r), id).Scan(&out.CurrentTime, &out.UpdatedAt)
	}
...
func (app *application) putProgress(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if kind == "movie" {
		err = app.pool.QueryRow(r.Context(), `insert into watch_progress(user_id,id_movie,current_time_seconds)
			select $1,m.id_movie,least($3,greatest(coalesce(m.duration,0)*60,0)) from movies m
			join titles t on t.id_title=m.id_title and t.deleted_at is null and t.published=true
			where m.id_movie=$2 and m.deleted_at is null
			and exists(select 1 from video_assets va where va.id_movie=m.id_movie and va.status='ready')
		...
		err = app.pool.QueryRow(r.Context(), `insert into watch_progress(user_id,id_episode,current_time_seconds)
			select $1,e.id_episode,least($3,greatest(coalesce(e.duration,0)*60,0)) from episodes e
			join seasons se on se.id_season=e.id_season and se.deleted_at is null
			join series s on s.id_series=se.id_series and s.deleted_at is null
			join titles t on t.id_title=s.id_title and t.deleted_at is null and t.published=true
			where e.id_episode=$2 and e.deleted_at is null
			and exists(select 1 from video_assets va where va.id_episode=e.id_episode and va.status='ready')
	...
	if err == pgx.ErrNoRows {
		jsonx.Error(w, http.StatusNotFound, "content not found")
		return
	}
	if err != nil {
		serverError(w, err)
...
func (app *application) continueWatching(w http.ResponseWriter, r *http.Request) {
	rows, err := app.pool.Query(r.Context(), `
		select kind,content_id,title_id,title,thumbnail,current_time_seconds,duration_seconds,updated_at from (
			...
			where wp.user_id=$1 and m.deleted_at is null and t.deleted_at is null and t.published=true and coalesce(m.duration,0)>0
				and exists(select 1 from video_assets va where va.id_movie=m.id_movie and va.status='ready')
			...
			where wp.user_id=$1 and e.deleted_at is null and se.deleted_at is null and s.deleted_at is null and t.deleted_at is null and t.published=true
				and exists(select 1 from video_assets va where va.id_episode=e.id_episode and va.status='ready')
